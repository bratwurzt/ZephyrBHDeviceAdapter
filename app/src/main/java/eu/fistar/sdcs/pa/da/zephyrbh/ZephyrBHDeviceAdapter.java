...
public class ZephyrBHDeviceAdapter extends Service
{
  ...
  private final IDeviceAdapter.Stub paEndpoint = new IDeviceAdapter.Stub()
  {
    ...
    public void connectDev(String devId) throws RemoteException
    {
      ...
      if (!deviceIsInTheList(devId, pairedDevices))
      {
        throw new IllegalArgumentException("The device " + devId + " is not paired or not supported by Device Adapter!");
      }
    ...
    public void addDeviceToWhitelist(String devId) throws RemoteException
    {
      ...
      if (!isValidDeviceId(devId) || deviceIsInTheList(devId, whitelist))
      {
        return;
      }
    ...
    {
      // If the device is already in the list, just do nothing
      if (!isValidDeviceId(devId) || deviceIsInTheList(devId, blacklist))
      {
        return;
      }
  ...
  public synchronized void receivedMeasurements(List<Observation> observations, ZephyrBHDevice device)
  {

    // Send the received measurement to the Protocol Adapter
    try
    {
      paApi.pushData(observations, new DeviceDescription(device));
    }
    catch (RemoteException e)
    {
      Log.e(LOGTAG_ZEPHYRBH_SERVICE, "Failed pushing device measurement:\n" + device.toString() + "\nFor device:\n" + device.toString());
  ...
  private boolean isSupportedDevice(String devId)
  {
    ...
    {
      if (devId.equals(tmpDev.getAddress()) && tmpDev.getName().startsWith(DEVICE_NAME_PREFIX))
      {
        return true;
      }
    }
